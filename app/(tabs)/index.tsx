...
export default function Index() {
  ...
  const handleSave = () => {
    ...
    if (selectedItem) {
      ...
      newData[selectedDate] = newData[selectedDate].map((item) =>
        item.id === selectedItem.id
          ? { ...item, title: inputText, amount: parseInt(inputAmount) || 0 }
          : item
      );
    } else {
      // 【新規追加モード】 今まで通りの処理
    ...
    closeModal(); // お掃除して閉じる（このあと作る関数です）
  };
  ...
  const openEditModal = (item: any) => {
    setSelectedItem(item);      // 「このアイテムをいじるよ」と記録
    setInputText(item.title);   // 入力欄に今のタイトルを入れる
    setInputAmount(item.amount.toString()); // 金額を入れる
    setModalVisible(true);      // モーダルを開く
  };
  ...
  const checkDelete = () => {
    Alert.alert(
      ...
      [
        ...
        {text:"いいえ",onPress: () =>console.log("キャンセルされました") ,style: "cancel"},
      ]
    );
  };
  ...
  const getMarkedDates = () => {
    ...
    Object.keys(scheduleData).forEach(date => {
      ...
      items.forEach(item => {
        // モード別のフィルター
        if (activeMode === 'calendar') dots.push({ key: item.id, color: item.color });
        else if (activeMode === 'todo' && item.isTodo) dots.push({ key: item.id, color: item.color });
        else if (activeMode === 'money' && item.amount > 0) dots.push({ key: item.id, color: item.color });
      });

      if (dots.length > 0) marked[date] = { dots: dots };
    });

    marked[selectedDate] = { ...marked[selectedDate], selected: true, selectedColor: '#ADE0EE' };
    return marked;
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Text style={[styles.tabText, activeMode === 'calendar' && styles.activeTabText]}>📅 カレンダー</Text>
        </TouchableOpacity>
        ...
        >
          <Text style={[styles.tabText, activeMode === 'todo' && styles.activeTabText]}>✅ ToDo</Text>
        </TouchableOpacity>
        ...
        >
          <Text style={[styles.tabText, activeMode === 'money' && styles.activeTabText]}>💰 家計簿</Text>
        </TouchableOpacity>
      </View>
        ...
        <View style={styles.calendarContainer}>
          ...
          <ScrollView style={styles.scheduleList}>
            {(() => {
              ...
              if (activeMode === 'todo') {
                const mainTasks = scheduleData[selectedDate]?.filter(item => item.isTodo) || [];
                const upcomingTasks = Object.keys(scheduleData)
                  .filter(date => date > selectedDate)
                  .sort()
                  .flatMap(date => scheduleData[date].map(task => ({ ...task, date })))
                ...
                return (
                  <View>
                    <Text style={styles.sectionTitle}>
                      {selectedDate === today ? '🔥 今日が締め切り' : `📅 ${selectedDate} 締め切り`}
                    </Text>
                    {mainTasks.length > 0 ? (
                      mainTasks.map(renderTodoItem)
                    ) : (
                      <Text style={styles.emptyText}>この日の締め切りはありません</Text>
                    )}

                    {upcomingTasks.length > 0 && (
                      <View style={styles.upcomingContainer}>
                        <View style={styles.divider} />
                        <Text style={styles.subSectionTitle}>⏳ その先のやること（余裕あり）</Text>
                        {upcomingTasks.map(task => (
                          <View key={task.id} style={styles.compactListItem}>
                            <Text style={styles.compactDate}>{task.date.split('-').slice(1).join('/')}</Text>
                ...
                return (
                  <View>
                    <Text style={styles.dateTitle}>
                      {activeMode === 'calendar' ? `${selectedDate} の予定` : `${selectedDate} の収支`}
                    </Text>
                    {scheduleData[selectedDate] ? (
                      scheduleData[selectedDate]
                        .filter(item => activeMode === 'calendar' ? true : item.amount > 0)
                        .map((item) => (
                          <TouchableOpacity key={item.id} style={styles.listItem} onPress={() => openEditModal(item)}>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{selectedDate} に追加</Text>
            <TextInput
              style={styles.input}
      ...
      {/* --- 追加：右下のプラスボタン --- */}
      <TouchableOpacity style={styles.fab} onPress={() => { closeModal(); setModalVisible(true); }}>
