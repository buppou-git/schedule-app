...
export default function HistoryAnalytics({ onClose }: { onClose?: () => void }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const wishReserved = wishlist.reduce((sum, item) => sum + (item.savedAmount || 0), 0);
  ...
  const { filteredItems, chartData, summary } = useMemo(() => {
    ...
    if (viewMode === "year") {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        let wInc = 0; let wExp = 0;
        (scheduleData[dateKey] || []).forEach(item => {
          wInc += getItemTotalIncome(item);
          wExp += getItemTotalExpense(item);
          items.push({ ...item, date: dateKey });
        });
        incomeData.push(wInc); expenseData.push(wExp);
      }
    }
  ...
  const availableTags = useMemo(() => {
    ...
    return Object.keys(tagMaster).filter(tag => layerFilters.includes(tagMaster[tag].layer));
  ...
  const availableCategories = useMemo(() => {
    ...
    if (layerFilters.length === 0) {
      ...
      layerFilters.forEach(l => {
        (quickMainTags[l] || quickMainTags["ALL_LAYERS"] || []).forEach(c => cats.add(c));
      });
    }
  ...
  const toggleFilter = (setFilterState: React.Dispatch<React.SetStateAction<string[]>>, value: string) => {
    ...
    setFilterState(prev => prev.includes(value) ? prev.filter(v => v !== value) : [...prev, value]);
  };
  ...
  const toggleLayerFilter = (layer: string) => {
    ...
    setLayerFilters(prev => {
      const next = prev.includes(layer) ? prev.filter(v => v !== layer) : [...prev, layer];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity onPress={onClose} style={[styles.backBtn, { zIndex: 1 }]}>
        ...
        <TouchableOpacity onPress={() => setIsFilterVisible(true)} style={[styles.filterTrigger, { zIndex: 1 }]}>
          <Ionicons name="filter" size={22} color={typeFilter !== "all" || layerFilters.length > 0 || tagFilters.length > 0 || categoryFilters.length > 0 ? "#007AFF" : "#8E8E93"} />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.assetCard}>
          ...
          <Text style={styles.assetAmount}>¥{totalSavings.toLocaleString()}</Text>
        </View>

        <View style={styles.viewModeTabs}>
          {(["year", "month", "week"] as ViewMode[]).map(mode => (
            <TouchableOpacity key={mode} style={[styles.modeTab, viewMode === mode && styles.modeTabActive]} onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setViewMode(mode); }}>
        ...
        <View style={styles.periodNav}>
          <TouchableOpacity onPress={() => {
            ...
            setReferenceDate(d);
          }}><Ionicons name="chevron-back" size={20} color="#8E8E93" /></TouchableOpacity>
          ...
          <TouchableOpacity onPress={() => {
            ...
            setReferenceDate(d);
          }}><Ionicons name="chevron-forward" size={20} color="#8E8E93" /></TouchableOpacity>
        </View>
        ...
        <View style={styles.summaryGrid}>
          <View style={styles.summaryItem}><Text style={styles.summaryLabel}>収入</Text><Text style={[styles.summaryValue, { color: "#34C759" }]}>+¥{summary.income.toLocaleString()}</Text></View>
          <View style={styles.summaryItem}><Text style={styles.summaryLabel}>支出</Text><Text style={[styles.summaryValue, { color: "#FF3B30" }]}>-¥{summary.expense.toLocaleString()}</Text></View>
          <View style={styles.summaryItem}><Text style={styles.summaryLabel}>収支</Text><Text style={[styles.summaryValue, { color: summary.balance >= 0 ? "#007AFF" : "#FF3B30" }]}>¥{summary.balance.toLocaleString()}</Text></View>
        </View>
            ...
            filteredItems.map((item, idx) => {
              ...
              return (
                <View key={idx} style={styles.historyRow}>
                  <View style={[styles.tagIndicator, { backgroundColor: color }]} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.itemTitle}>{item.title}</Text>
                    <Text style={styles.itemSub}>
                      {item.date.replace(/-/g, "/")} | {tag} {item.category ? `(${item.category})` : ""}
                    </Text>
                  </View>
                  <Text style={[styles.itemAmount, { color: getItemTotalIncome(item) > 0 ? "#34C759" : "#1C1C1E" }]}>
                    {getItemTotalIncome(item) > 0 ? "+" : "-"}¥{(getItemTotalIncome(item) || getItemTotalExpense(item)).toLocaleString()}
                  </Text>
                </View>
              );
            })
          ...
          <TouchableWithoutFeedback><View style={styles.filterModal}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
              <View style={styles.filterHeader}>
                ...
                <TouchableOpacity onPress={() => { setTypeFilter("all"); setLayerFilters([]); setTagFilters([]); setCategoryFilters([]); }}>
              ...
              <View style={styles.filterGroup}>
                {(["all", "expense", "income"] as const).map(type => (
                  <TouchableOpacity key={type} style={[styles.filterBtn, typeFilter === type && styles.filterBtnActive]} onPress={() => setTypeFilter(type)}>
              ...
              <Text style={styles.filterSubTitle}>カレンダーの種類（複数選択可）</Text>
              <View style={styles.wrapContainer}>
                <TouchableOpacity style={[styles.tagBtn, layerFilters.length === 0 && styles.tagBtnActive]} onPress={() => { setLayerFilters([]); setTagFilters([]); }}>
                  <Text style={layerFilters.length === 0 ? styles.tagBtnTextActive : styles.tagBtnText}>すべて</Text>
                </TouchableOpacity>
              ...
              {availableTags.length === 0 ? (
                <Text style={styles.guideText}>※カレンダーの種類を選択すると、関連する属性が表示されます。</Text>
              ) : (
                <View style={styles.wrapContainer}>
                  <TouchableOpacity style={[styles.tagBtn, tagFilters.length === 0 && styles.tagBtnActive]} onPress={() => setTagFilters([])}>
                    <Text style={tagFilters.length === 0 ? styles.tagBtnTextActive : styles.tagBtnText}>すべて</Text>
                  </TouchableOpacity>
              ...
              {availableCategories.length > 0 && (
                <>
                  <Text style={styles.filterSubTitle}>支出カテゴリ（複数選択可）</Text>
                  <View style={styles.wrapContainer}>
                    <TouchableOpacity style={[styles.tagBtn, categoryFilters.length === 0 && styles.tagBtnActive]} onPress={() => setCategoryFilters([])}>
                      <Text style={categoryFilters.length === 0 ? styles.tagBtnTextActive : styles.tagBtnText}>すべて</Text>
                    </TouchableOpacity>
              ...
              <TouchableOpacity style={styles.closeFilterBtn} onPress={() => setIsFilterVisible(false)}>
...
const styles = StyleSheet.create({
  ...
  assetAmount: { color: "#FFF", fontSize: 32, fontWeight: "900", letterSpacing: 1 },
  viewModeTabs: { flexDirection: "row", backgroundColor: "#E5E5EA", marginHorizontal: 20, borderRadius: 12, padding: 4, marginBottom: 15 },
  modeTab: { flex: 1, paddingVertical: 8, alignItems: "center", borderRadius: 8 },
  modeTabActive: { backgroundColor: "#FFF", shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 2, elevation: 2 },
  ...
  sectionTitle: { fontSize: 15, fontWeight: "800", marginBottom: 15, color: "#1C1C1E" },
  historyRow: { flexDirection: "row", alignItems: "center", paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: "#F2F2F7" },
  ...
  filterSubTitle: { fontSize: 13, fontWeight: "bold", color: "#8E8E93", marginBottom: 12 },
  filterGroup: { flexDirection: "row", gap: 10, marginBottom: 20 },
  filterBtn: { flex: 1, paddingVertical: 12, borderRadius: 12, backgroundColor: "#F2F2F7", alignItems: "center" },
