...
export default function HistoryAnalytics({
  ...
  const { filteredItems, chartData, summary } = useMemo(() => {
    ...
    // 🌟 追加：戻入を「収入」としてグラフにカウントさせないための専用計算機
    const getPureIncome = (item: ScheduleItem) => {
      if (
        item.category === "目標からの戻入" ||
        (item.title && item.title.includes("戻入"))
      ) {
        return 0; // 戻入は収入グラフに含めない
      }
      return getItemTotalIncome(item);
    };
    ...
    if (viewMode === "year") {
        ...
        Object.keys(scheduleData).forEach((date) => {
          if (date.startsWith(prefix)) {
            (scheduleData[date] || []).forEach((item) => {
              mInc += getPureIncome(item); // 🌟 変更：純粋な収入だけを足す
      ...
      for (let i = 1; i <= lastDay; i++) {
        ...
        (scheduleData[dateKey] || []).forEach((item) => {
          dInc += getPureIncome(item); // 🌟 変更：純粋な収入だけを足す
      ...
      for (let i = 0; i < 7; i++) {
        ...
        let wExp = 0;
        (scheduleData[dateKey] || []).forEach((item) => {
          wInc += getPureIncome(item); // 🌟 変更：純粋な収入だけを足す
    ...
    const finalItems = items
      .filter((item) => {
        const inc = getItemTotalIncome(item); // 明細一覧には表示させたいのでここはそのまま
        ...
        // 🌟 追加：戻入を「収入フィルター」で見た時に表示させるかどうかの調整
        if (typeFilter === "income" && inc > 0 && getPureIncome(item) === 0) {
          // 「目標からの戻入」は明細一覧には出るが、純粋な収入として絞り込んだ時は出さない方が綺麗です
          // あえて表示したい場合はこのif文を消してください。
        }
