...
export default function ScheduleModal({
    ...
    const currentQuickTags = useMemo(() => {
        return quickMainTags[selectedLayer] || quickMainTags["ALL_LAYERS"] || ["食費", "交通", "日用品", "交際費", "趣味", "その他"];
    ...
    const saveQuickTag = async () => {
        ...
        if (!newTags[selectedLayer]) {
            newTags[selectedLayer] = [...(quickMainTags["ALL_LAYERS"] || ["食費", "交通", "日用品", "交際費", "趣味", "その他"])];
        }
    ...
    const handleRegisterTag = async () => {
        if (!tagInput.trim()) return Alert.alert("エラー", "属性名を入力してください");
    ...
    useEffect(() => {
        if (visible) {
            ...
            if (selectedItem) {
                ...
                if (selectedItem.startDate) setStartDate(new Date(selectedItem.startDate));
    ...
    const executeDelete = (deleteFuture: boolean) => {
        ...
        if (deleteFuture && selectedItem?.recurringGroupId) {
            ...
            Object.keys(newData).forEach((date) => {
                newData[date] = newData[date].filter((item: any) => item.id !== selectedItem!.id);
            });
        }
    ...
    const executeSave = async (updateFuture: boolean) => {
        ...
        await AsyncStorage.setItem("readingMasterData", JSON.stringify(newReadingMaster));
        ...
        const durationDays = Math.round((endNoTime.getTime() - startNoTime.getTime()) / (1000 * 60 * 60 * 24));
        ...
        if (selectedItem) {
            if (updateFuture && selectedItem.recurringGroupId) {
                ...
                Object.keys(newData).forEach((date) => {
                    newData[date] = newData[date].filter((i: any) => i.id !== selectedItem.id);
                });
                            ...
                            <View style={styles.timeSection}>
                                <View style={styles.timePreviewRow}>
                                    ...
                                    <Text style={[styles.timePreviewText, { color: uiThemeColor }]}>
                                        {isAllDay 
                                            ? (startDate.getTime() === endDate.getTime() ? "終日" : `${startDate.getMonth()+1}/${startDate.getDate()} 〜 ${endDate.getMonth()+1}/${endDate.getDate()} 終日`)
                                    ...
                                    <View style={[styles.newTagInputArea, { borderColor: uiThemeColor }]}>
                                        <View style={styles.tagInputRow}>
                                            ...
                                            <View style={styles.tagActionIcons}>
                                                {tagMaster?.[tagInput] && (
                                                    <TouchableOpacity onPress={() => handleDeleteTag(tagInput)} style={styles.iconBtn}>
                                                        <Ionicons name="trash-outline" size={20} color="#FF3B30" />
                                                    </TouchableOpacity>
                                                )}
                                                <TouchableOpacity onPress={handleRegisterTag} style={styles.iconBtn}>
                                                    <Ionicons name="checkmark-circle" size={28} color={uiThemeColor} />
                                                </TouchableOpacity>
                                            </View>
                                        </View>
                                        ...
                                        >
                                            {currentQuickTags.map((cat, index) => (
                                                <TouchableOpacity
                                                    ...
                                                    onLongPress={() => handleLongPressQuickTag(index, cat)}
                                                >
                                                    <Text
                                                        style={[
                                                            styles.layerChipText,
                ...
                >
                    <View style={styles.editCardModal}>
                        ...
                        <View style={[styles.editActionRow, { justifyContent: "center", marginTop: 10 }]}>
                            <TouchableOpacity
                                style={[styles.saveBtnModal, { backgroundColor: uiThemeColor, width: "100%", alignItems: "center" }]}
...
const styles = StyleSheet.create({
    ...
    saveBtnModal: { paddingHorizontal: 25, paddingVertical: 12, borderRadius: 15 },
